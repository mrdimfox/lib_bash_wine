...
function get_gecko_32_bit_msi_name_from_wine_prefix {
    ...
    if [[ "${wine_arch}" == "win32" ]]; then
        msi_file_name="$(cat "${wine_prefix}/drive_c/windows/system32/appwiz.cpl" | strings | grep "wine_gecko" | grep ".msi")"
    else
        msi_file_name="$(cat "${wine_prefix}/drive_c/windows/syswow64/appwiz.cpl" | strings | grep "wine_gecko" | grep ".msi")"
    fi
